"""
该代码适合多人按键使用，使用的是ctypes库，这个相对比较复杂。
按下左Ctrl键后，会自动运行。按下ESC键后，会停止运行。再次按CTRL后又会重新运行。
目前有问题

因为所有窗口都叫“魔兽世界”，怎么分判？我看到有的视频上显示魔兽世界的窗口后面显示了
1. 只能在当前窗口激活，
2. 还没有运行一会儿，就显示IDE内存不够了，应该是这个程序有内存溢出的问题。
"""
...
# 标志变量，控制main函数的运行状态
IS_RUNNING = [False] * len(hwnds)
main_threads = [None] * len(hwnds)
...
def main(index):
    ...
    for attempt in range(RETRY_ATTEMPTS):
        ...
        if hwnd:
            ...
            if hwnd not in hwnds:
                # 如果hwnds[0]为None，优先存储到hwnds[0]
                if hwnds[0] is None:
                    hwnds[0] = hwnd
                    print(f"找到第【1】号 {hwnds[index]} 窗口，窗口句柄为：{hex(hwnds[0])}")
                    break
                elif None in hwnds:
                    hwnds[hwnds.index(None)] = hwnd
                    print(f"找到第【{hwnds.index(hwnd)}】号窗口：{hwnds[index]}，窗口句柄为：{hex(hwnd)}")
                    break
                else:
                    print(f"所有位置已被占用，无法存储新的窗口句柄：{hex(hwnd)}")
                    break
            else:
                print(f"窗口句柄 {hex(hwnd)} 已经存在于hwnds中，无法存储。")
                break
        else:
            print(f"未找到窗口：{hwnds[index]}，尝试第 {attempt + 1} 次重试...")
            time.sleep(1)
    ...
    if hwnds[index]:
        ...
        while IS_RUNNING[index]:
            ...
            press_key(hwnds[index], KEYS_TO_PRESS)
            # print(i)
            if IS_SLEEP_RANDOM:
                time.sleep(random.randint(*SLEEP_TIME_RANGE))
    else:
        print(f"经过 {RETRY_ATTEMPTS} 次尝试后仍未找到窗口：{hwnds[index]}")


def on_press(key):
    try:
        global IS_RUNNING, main_threads, hwnds
        if key == keyboard.Key.ctrl_l:  # 检测左Ctrl键
            print(f"按下了按键：{key}")
            print(len(hwnds), hwnds)
            for i in range(len(hwnds)):
                if IS_HWND[i]:
                    if not IS_RUNNING[i]:
                        IS_RUNNING[i] = True
                        print(f"开始运行第{len(hwnds)}个 {WINDOW_TITLE} 窗口的main函数")
                        main_threads[i] = threading.Thread(target=main, args=(i,))
                        main_threads[i].start()
            ...
            for i in range(len(hwnds)):
                if IS_HWND:
                    if IS_RUNNING[i]:
                        IS_RUNNING[i] = False
                        print(f"停止运行第{len(hwnds)}个 {WINDOW_TITLE} 窗口的的main函数")
                        if main_threads[i] and main_threads[i].is_alive():
                            main_threads[i].join(timeout=1)
    except Exception as e:
        print(f"处理按键时出错：{e}")
