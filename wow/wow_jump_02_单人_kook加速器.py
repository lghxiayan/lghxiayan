...
使用进程名（例如 wow.exe）来查找目标窗口，你可以使用一些系统级别的库来实现。
在 Python 中，你可以使用 psutil 库来获取进程信息，并结合 pygetwindow 或 win32gui 库来查找窗口
"""
...
def find_wow_window():
    def callback(hwnd, extra):
        ...
        if class_name == "GxWindowClassD3d" or "World of Warcraft" in window_title:
...
def Find_window_by_process_name(process_name):
    # 将进程名转换为小写以便进行不区分大小写的比较
    process_name_lower = process_name.lower()
    for proc in psutil.process_iter(['pid', 'name']):
        if proc.info['name'].lower() == process_name_lower:
            pid = proc.info['pid']
            print(f"找到进程 {process_name}，PID: {pid}")  # 找到进程 wow.exe，PID: 19276
            for window in gw.getAllWindows():
                hwnd = window._hWnd
                print(hwnd)
                window_pid = get_pid_from_hwnd(hwnd)
                print(f"进程ID: {window_pid}")
                if window_pid == pid:
                    print(f"找到进程ID: {window_pid}，窗口句柄为：{hex(window._hWnd)},HEX为{hwnd}")  # 找到进程ID: 19276，窗口句柄为：0xa1b2c
                    return hex(window._hWnd)  # 返回窗口句柄
    return None
...
def press_key(hwnd, keys=KEYS_TO_PRESS, interval=INTERVAL_BETWEEN_KEYS):
    ...
    """
    print(f"press_key函数内部运行，99行。hwnd为{hwnd}")
    # if not isinstance(hwnd, ctypes.wintypes.HANDLE):
    #     raise TypeError("hwnd must be a PyHWND object.中文")
    try:
        ...
        set_keyboard_state(ctypes.byref(key_board_state_buffer))
        ...
        print(f"模拟按键操作时出错：{e}")  # 结果为 模拟按键操作时出错：The object is not a PyHANDLE object


def main():
    result = find_wow_window()
    print(f"find_wow_window结果为：{result}")

    wow_window = Find_window_by_process_name(PROCESS_NAME)
    if wow_window:
        print(f"找到窗口句柄为{wow_window}")
    else:
        print(f"未找到窗口")
    ...
    for attempt in range(RETRY_ATTEMPTS):
        ...
        if hwnd:
            print(f"找到窗口句柄为：{hwnd}")
            break
        else:
            print(f"未找到窗口，尝试第 {attempt + 1} 次重试...")
            time.sleep(1)
    ...
    if hwnd:
        ...
        print(f"经过 {RETRY_ATTEMPTS} 次尝试后仍未找到窗口句柄。")
