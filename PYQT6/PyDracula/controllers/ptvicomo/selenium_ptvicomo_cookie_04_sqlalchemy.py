...
def load_config(file_path):
    ...
    try:
        ...
        raise ValueError(f"无法读取配置文件 {file_path}: {e}")

    try:
        config_return_data = {
            'HEAD_LESS': config.getboolean('DEFAULT', 'HEAD_LESS'),
            'DB_TYPE': config.get('DATABASE', 'DB_TYPE'),
            'DB_CONFIG': ast.literal_eval(config.get('DATABASE', 'DB_CONFIG')),
            'TABLE_NAME': config.get('DATABASE', 'TABLE_NAME'),
            'WEB_COOKIE': ast.literal_eval(config.get('DEFAULT', 'WEB_COOKIE')),
            'WEBSITE_URL': config.get('DEFAULT', 'WEBSITE_URL'),
            'WEBSITE_MAIN_URL': config.get('DEFAULT', 'WEBSITE_MAIN_URL'),
            'WAIT_TIMEOUT': config.getint('DEFAULT', 'WAIT_TIMEOUT'),
            'CURRENT_ACTION': ast.literal_eval(config.get('TRADE', 'CURRENT_ACTION')),
            'SALE_NUMBER': config.getint('TRADE', 'SALE_NUMBER'),
            'BUY_NUMBER': config.getint('TRADE', 'BUY_NUMBER'),
            'PROFIT_MARGIN': config.getint('TRADE', 'PROFIT_MARGIN'),
            'SAVE_PAGE': config.getboolean('DEFAULT', 'SAVE_PAGE'),
        }
        return config_return_data
...
HEAD_LESS = config_data['HEAD_LESS']
DB_TYPE = config_data['DB_TYPE']
DB_CONFIG = config_data['DB_CONFIG']
TABLE_NAME = config_data['TABLE_NAME']
WEB_COOKIE = config_data['WEB_COOKIE']
WEBSITE_URL = config_data['WEBSITE_URL']
WEBSITE_MAIN_URL = config_data['WEBSITE_MAIN_URL']
WAIT_TIMEOUT = config_data['WAIT_TIMEOUT']
CURRENT_ACTION = config_data['CURRENT_ACTION']
SALE_NUMBER = config_data['SALE_NUMBER']
BUY_NUMBER = config_data['BUY_NUMBER']
PROFIT_MARGIN = config_data['PROFIT_MARGIN']
...
logging_config_file_path = os.path.join(script_dir, 'logging_ptvicomo.conf')
if not os.path.exists(logging_config_file_path):
    print(f"配置文件 {logging_config_file_path} 不存在")
logging.config.fileConfig(logging_config_file_path, encoding='utf-8')
