...
def setup_logging():
    try:
        ...
        # 定义日志文件名
        log_file_name = f"wowjump_log_{time.strftime('%Y%m%d')}.log"
        # log_file_name = f"wowjump_log_{time.strftime('%Y%m%d_%H%M%S')}.log"
        # 构造保存日志文件目录的路径
        ...
        # 使用绝对路径来加载日志配置文件
        config_file_path = os.path.join(current_dir, 'logging_wowjump.conf').replace('\\', '\\\\')
        logging.config.fileConfig(config_file_path, encoding='utf-8', defaults={'logfilename': log_file_path})
        my_logger = logging.getLogger('wowjump_log')
        my_logger.setLevel(CONFIG.get('LOG_LEVEL', logging.INFO))
        return my_logger
...
class WoWKeyPresser:
            ...
            for _ in range(interval):
                for key in keys:
                    ...
                    lparam = win32api.MAKELONG(
                        0, _user32.MapVirtualKeyA(key_code, 0)) | 0x00000001
                    ctypes.windll.user32.PostMessageW(
                        hwnd, win32con.WM_KEYDOWN, key_code, lparam)
                    time.sleep(0.1)
                    ctypes.windll.user32.PostMessageW(
                        hwnd, win32con.WM_KEYUP, key_code, lparam | 0xC0000000)
                    time.sleep(0.1)
            _user32.GetKeyboardState(ctypes.byref(old_keyboard_state))
            _user32.AttachThreadInput(current_thread_id, hwnd, False)
        except Exception as e:
            logger.error(self.safe_log_message(f"模拟按键操作时出错:{e}"))

    def on_press(self, key):
        try:
            if key == keyboard.Key.ctrl_l:  # 检测左Ctrl键
                logger.info(self.safe_log_message(f"按下了按键:{key}"))
                ...
                if self.is_running:
                    self.is_running = False
                    logger.info(self.safe_log_message("停止运行main函数"))
                    if self.main_thread and self.main_thread.is_alive():
                        self.main_thread.join(timeout=1)
        except Exception as e:
            logger.error(self.safe_log_message(f"处理按键时出错:{e}"))

    @staticmethod
    def on_release(key):
        try:
            if key == keyboard.Key.esc:  # 检测esc键退出监听
                logger.info(f"释放了按键:{key}")
        except Exception as e:
            logger.error(f"处理按键释放时出错:{e}")
    ...
    def main(self):
        ...
        for attempt in range(self.config["RETRY_ATTEMPTS"]):
            ...
            if hwnd:
                logger.info(self.safe_log_message(
                    # 找到窗口:魔兽世界,窗口句柄为:0x31638
                    f"找到窗口:{self.config['WINDOW_TITLE']},窗口句柄为:{hex(hwnd)}"))
                break
            else:
                logger.info(self.safe_log_message(
                    f"未找到窗口:{self.config['WINDOW_TITLE']},尝试第 {attempt + 1} 次重试..."))
                time.sleep(1)
        if hwnd:
            while self.is_running:
                ...
                if self.is_sleep_random:
                    time.sleep(random.randint(
                        *self.config["SLEEP_TIME_RANGE"]))
        else:
            logger.error(self.safe_log_message(
                f"经过 {self.config['RETRY_ATTEMPTS']} 次尝试后仍未找到窗口:{self.config['WINDOW_TITLE']}"))
