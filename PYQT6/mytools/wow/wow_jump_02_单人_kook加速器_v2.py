...
class WoWKeyPresser:
    ...
    def find_wow_window(self):
        def callback(hwnd, extra):
            ...
            if class_name == WOW_CLASS_NAME or WINDOW_TITLE in window_title:
    ...
    def press_key(self, hwnd, keys=KEYS_TO_PRESS, interval=INTERVAL_BETWEEN_KEYS):
        """
        模拟按键操作
        :param hwnd: 窗口句柄
        :param keys: 要按下的键列表
    ...
    def main(self):
        ...
        for attempt in range(RETRY_ATTEMPTS):
            ...
            # print(f"hwnd: {hwnd}")
            if hwnd:
                print(f"找到窗口：{WINDOW_TITLE}，窗口句柄为：{hex(hwnd)}")  # 找到窗口：魔兽世界，窗口句柄为：0x31638
                break
            else:
        ...
        if hwnd:
            while self.is_running:
                ...
                self.press_key(hwnd, KEYS_TO_PRESS)
                # print(i)
