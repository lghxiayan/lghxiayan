...
# 标志变量，控制main函数的运行状态
IS_RUNNING = [False] * len(WINDOW_TITLES)
main_threads = [None] * len(WINDOW_TITLES)
...
def main(index):
    ...
    for attempt in range(RETRY_ATTEMPTS):
        hwnd = get_window_handle(WINDOW_TITLES[index])
        print(hwnd)
        if hwnd:
            print(f"找到窗口：{WINDOW_TITLES[index]}，窗口句柄为：{hex(hwnd)}")
            break
        else:
            print(f"未找到窗口：{WINDOW_TITLES[index]}，尝试第 {attempt + 1} 次重试...")
            time.sleep(1)
    ...
    if hwnd:
        ...
        print(f"经过 {RETRY_ATTEMPTS} 次尝试后仍未找到窗口：{WINDOW_TITLES[index]}")


def on_press(key):
    try:
        global IS_RUNNING, main_threads
        if key == keyboard.Key.ctrl_l:  # 检测左Ctrl键
            print(f"按下了按键：{key}")
            for i in range(len(WINDOW_TITLES)):
                if not IS_RUNNING[i]:
                    IS_RUNNING[i] = True
                    print(f"开始运行窗口 {WINDOW_TITLES[i]} main函数")
            ...
            for i in range(len(WINDOW_TITLES)):
                if IS_RUNNING[i]:
                    IS_RUNNING[i] = False
                    print(f"停止运行窗口 {WINDOW_TITLES[i]} 的main函数")
